...
class Reader:
    ...
    def inventory_active_mode(self) -> Iterator[Response]:
        while True:
            ...
            try:
                ...
                print(f"[DEBUG] Parse error: {e} | Raw: {hex_readable(raw_response)}")
                continue
    ...
    def set_power(self, power: int) -> Response:  # 8.4.6 Set Power
        ...
        # Retry: skip stale active-mode tag frames
        for _ in range(10):
            ...
            try:
                resp = Response(raw)
                if resp.command == CMD_SET_READER_POWER:
                    return resp
            except (ValueError, IndexError):
                continue
        raise RuntimeError("Tidak dapat set power - reader tidak merespons")
    ...
    def work_mode(self) -> WorkMode:  # 8.4.10 Get WorkMode
        ...
        for _ in range(10):
            ...
            try:
                resp = Response(raw)
                if resp.command == CMD_GET_WORK_MODE:
                    return WorkMode(resp.data)
            except (ValueError, IndexError):
                continue
        raise RuntimeError("Tidak dapat baca work mode - reader tidak merespons")
    ...
    def set_work_mode(self, work_mode: WorkMode) -> Response:  # 8.4.9 Set WorkMode
        ...
        for _ in range(10):
            ...
            try:
                resp = Response(raw)
                if resp.command == CMD_SET_WORK_MODE:
                    return resp
            except (ValueError, IndexError):
                continue
        raise RuntimeError("Tidak dapat set work mode - reader tidak merespons")
